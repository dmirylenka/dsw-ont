...
import logging
from dswont.wikiapi import CategoryRelationCache


# Bloody mess
# TODO: Get rid of this file.

ROOT_CATEGORY_COMPUTING = "http://dbpedia.org/resource/Category:Computing"
DEFAULT_MAX_DEPTH = 7
...
class TopicFeatures(object):
    ...
    Usage example:
        ...
        >>>     return all([word[0].isupper() for word in uri_to_title(node)
        .split()])
        >>> 
        >>> features = TopicFeatures({'n_words': n_words, 'is_capitalized':
        is_capitalized_ftr})
        >>> 
        >>> nodes = ['http://dbpedia
        .org/resource/Category:Mac_OS_X_backup_software',
...
class CategorySelection(object):
    ...
    def __init__(self, root, classifier: TopicClassifier=None, max_depth=None,
        ...
        if not relation_cache:
            raise ValueError(
                "Cannot create CategorySelection without relation_cache.")
    ...
    def _compute_children(self, node):
        ...
        if children is None:
            #             children = set(self._wiki_graph.get_subcats(node)
            # or [])
    ...
    def _classify_node(self, node):
        ...
        #         logging.debug(prefix + "classifying the node '{}'".format(
        # uri_to_title(node)))
    ...
    def relevant_nodes(self):
        return [child for child, parent in
                self._bfs(should_report=self.is_relevant)]
...
class MajorityClassClassifier(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        self._most_frequent_element = \
        max(frequencies, key=lambda item: item[1])[0]
...
class TrainingDataSelection(object):
    ...
    def sample_paths_through_from_anywhere(self, n_nodes, fixed_length=True,
    ...
    def sample_relations_with_children(self, n_nodes, max_children=None,
                                       with_paths=True,
                                       filter_parent=None, filter_child=None,
                                       random_seed=29121985):
        random.seed(random_seed)
        result = collections.OrderedDict()
        selection = self.selection
        all_nodes = [child for child, parent in
                     selection._bfs(should_schedule=selection.is_relevant)]
        nodes_with_children = [node for node in all_nodes if
                               selection.get_children(node)]
        ...
        def get_n_children(node, path):
            ...
            children_off_path = [child for child in children if
                                 child not in path]
            ...
            return list(
                itertools.islice(filter_children(children), 0, n_children))
        ...
        for sampled_node in nodes_with_children:
            ...
            if sampled_node in visited:
                continue
            path = filter_parents(
                filter_visited_and_visit(get_path(sampled_node)))
            for node in path:
...
class TrainingDataProcessing(object):
    ...
    def save_topic_labels(self, topics, labels, filename,
        ...
        with open(filename, 'r', encoding='utf-8') as file:
            for line in file.readlines():
                try:
                    ...
                    logging.info(
                        "Invalid label, ignoring the line: {}".format(line))
        return result
...
def frac_children_with_parents_in_graph_feature(selection: CategorySelection,
                                                node):
    children_total = selection._children.get(node, [])
...
# Not used
def heuristic_score_feature_fn(selection: CategorySelection, node):
    """Implements the score given by our old heuristic algorithm.
    
    Monstrous."""

    def get_parents(aselection, anode):
        if anode in aselection._parents:
            return aselection._parents[anode]
        else:
            return []

    def heuristic_score_computation(relevance_factor, decay_factor):

        def score_from_parent_scores(parent_scores, node_depth):
            """Returns the score of the node given its parent nodes' scores."""
            sum_of_scores = sum(parent_scores)
            n_zero_scores = sum(1 for score in parent_scores if score == 0)
            n_nonzero_scores = len(parent_scores) - n_zero_scores
            relevance_boost = relevance_factor * pow(decay_factor, node_depth)
            relevance_boosted_n_scores = n_nonzero_scores + \
                                         (n_zero_scores / relevance_boost)
            return sum_of_scores / relevance_boosted_n_scores

        def compute_score(anode, score_cache={}):

            # print("Computing the score for '{}'".format(uri_to_title(anode)))
            if anode == selection._root:
                score = 1
            elif not selection.get_parents(anode):
                score = 0
            elif anode in score_cache:
                score = score_cache[anode]
            else:
                parents = [parent for parent in get_parents(selection, anode)
                           if not selection._is_ancestor(parent, anode)]
                if not parents:
                    return 0
                # print("Parents: '{}'".format(', '.join(uri_to_title(parent)
                #                                   for parent in parents)))
                parent_scores = [compute_score(parent) for parent in parents]
                depth = selection.get_depth(anode)
                score = score_from_parent_scores(parent_scores, depth)
            # print("Score for '{}' : {}".format(uri_to_title(anode), score))
            score_cache[anode] = score
            return score

        return compute_score(node)

    return heuristic_score_computation


def heuristic_score_feature_fn_for_learning(relevance_factor, decay_factor):
    def heuristic_score_feature(selection: CategorySelection, node):
        return heuristic_score_feature_fn(selection, node)(relevance_factor,
                                                           decay_factor)
...
# Ad hoc procedure, used once
def save_merged_ground_truth_data(full_selection, filename=util.resource(
    default_ground_truth_data_file)):
...
def depth_based_selection(root=ROOT_CATEGORY_COMPUTING,
                          max_depth=DEFAULT_MAX_DEPTH):
    relation_cache = CategoryRelationCache(
        subcat_index_file=wiki.WIKI_SUBCAT_INDEX_FILE,
        supercat_index_file=wiki.WIKI_SUPERCAT_INDEX_FILE)
...
default_features = TopicFeatures({
    ...
    #  'frac_children_with_parents_in_graph':
    # frac_children_with_parents_in_graph_feature,
...
default_classifier_params = {'C': 3, 'kernel': 'linear',
                             'class_weight': {0: 1, 1: 1}}

default_param_grid = [
    {'C': [3], 'kernel': ['linear'], 'class_weight': [{0: 1, 1: 1}]}]
    ...
    for train_idx, test_idx in outer_cross_validation(classes):
        ...
        if training_size is not None:
            ...
            if training_size and actual_training_size < training_size:
                print(
                    "Warning: Training set size {} is smaller than requested: "
                    "{}."
                    .format(actual_training_size, training_size))
            sampled_train_idx = np.random.choice(train_idx, training_size,
                                                 replace=False)
        results.append([])
        inner_cross_validation_object = inner_cross_validation(
            classes[sampled_train_idx]) \
        ...
        for measure in evaluation_measures:
            score = evaluate_classifier(clf, topics[test_idx],
                                        classes[test_idx], measure)
...
def report_shortest_path(selection, topic):
    return TrainingDataSelection(selection)._shortest_path_from_root(
        to_category_uri(topic));
